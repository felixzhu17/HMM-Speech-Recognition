...
def stft(y, sr, window_ms=10, overlap_pct=0.25):
    ...
    return np.array(
        [
            np.fft.rfft(w * y[i : i + window_size])
...
def mel_filter_bank(sr, n_fft, n_mels):
    ...
    mels = np.linspace(
        mel_min, mel_max, num=n_mels + 2
    )  # Equally spaced in Mel scale, but add two for the endpoints
    ...
    bins = np.floor(total_n_fft * freqs / sr).astype(
        int
    )  # Which bin does each frequency correspond to? Frequency distinction between bins = sr/n_fft

    fbank = np.zeros(
        [n_mels, n_fft]
    )  # Initialize empty filter bank matrix with rows = number of mel filters, columns = total number of FFT bins
...
def get_mfcc(y, sr, window_ms=10, overlap_pct=0.25, mel_banks=20, n_mfcc=12):
    ...
    )  # Power = amplitude^2 is often taken as it aligns more with human hearing
    ...
    return dct(log_mel_spectrogram, type=2, axis=1, norm="ortho")[
        :, 1 : (n_mfcc + 1)
    ]  # Keep first few coefficients, except the first, as others are usually not informative
