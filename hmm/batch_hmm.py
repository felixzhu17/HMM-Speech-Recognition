...
class BatchStableGMMHMM:
    ...
    def _init_mu(self, obs):
        ...
        self.mu = flattened_obs[:, subset].T
    ...
    def m_step(self, obs):
        ...
        for s in range(self.n_states):
            self.mu[s,:] = (gamma_s[:, s, :].reshape(n_batches, n_observations, 1) * obs).sum(axis=(0, 1))/gamma_s[:, s, :].sum()

        # Covariance is the weighted sum of the outer product of the difference of each observation from the mean
        for s in range(self.n_states):
            diff = obs - self.mu[s, :]
            self.covs[s, :, :] = (np.matmul(gamma_s[:, s, :].reshape(n_batches, 1, n_observations) * diff.transpose(0, 2, 1), diff)).sum(axis=0) / gamma_s[:, s, :].sum()
