...
class BatchStableGMMHMM:
    ...
    def _init_mu(self, obs):
        ...
        flattened_obs = obs.reshape(obs.shape[2], -1).T

        # Apply K-Means to find initial mean estimates
        kmeans = KMeans(
            n_clusters=self.n_states, n_init=10, random_state=self.random_state
        )
        kmeans.fit(flattened_obs)
    ...
    def _init_covs(self, obs):
        ...
        flattened_obs = obs.reshape(obs.shape[2], -1)
    ...
    def e_step(self, obs):
        ...
        self.xi = np.zeros(
            (n_batches, self.n_states, self.n_states, n_observations - 1)
        )
        for batch in range(n_batches):
            for t in range(n_observations - 1):  # Loop through every time step
                for i in range(self.n_states):  # Loop through every start state
                    ...
                    self.xi[batch, i, :, t] = (
                        self.alpha[batch, i, t]
                        + self.log_A[i, :]
                        + emission_log_likelihood[batch, :, t + 1]
                        + self.beta[batch, :, t + 1]
                    )
    ...
    def m_step(self, obs):
        ...
        if not self.deterministic_start:
            self.log_prior = np.logaddexp.reduce(
                (self.gamma[:, :, 0] - np.log(n_batches))
            )  # Take average of exponential of log probabilities
        ...
        for s in range(self.n_states):
            self.mu[s, :] = (
                gamma_s[:, s, :].reshape(n_batches, n_observations, 1) * obs
            ).sum(axis=(0, 1)) / gamma_s[:, s, :].sum()
        ...
        for s in range(self.n_states):
            ...
            self.covs[s, :, :] = (
                np.matmul(
                    gamma_s[:, s, :].reshape(n_batches, 1, n_observations)
                    * diff.transpose(0, 2, 1),
                    diff,
                )
            ).sum(axis=0) / gamma_s[:, s, :].sum()
            self.covs[s, :, :] += (
        ...
        for i in range(self.n_states):
            for j in range(self.n_states):
                self.log_A[i, j] = np.logaddexp.reduce(
                    self.xi[:, i, j, :].flatten()
                ) - np.logaddexp.reduce(self.gamma[:, i, :-1].flatten())
        ...
        for batch in range(n_batches):
            for s in range(self.n_states):
                B[batch, s, :] = st.multivariate_normal.pdf(
                    obs[batch], mean=self.mu[s, :], cov=self.covs[s, :, :]
                )
        return np.log(B + 1e-10)
    ...
    def forward(self, obs, emission_log_likelihood=None):
        ...
        for t in range(1, n_observations):
            ...
            alpha[:, :, t] = emission_log_likelihood[:, :, t] + np.logaddexp.reduce(
                self.log_A.T + alpha[:, np.newaxis, :, t - 1], axis=2
            )  # Insert additional axis to broadcast along the rows of the transition matrix
    ...
    def backward(self, obs, emission_log_likelihood=None):
        ...
        for t in range(n_observations - 1)[::-1]:
            ...
            beta[:, :, t] = np.logaddexp.reduce(
                beta[:, np.newaxis, :, t + 1]
                + emission_log_likelihood[:, np.newaxis, :, t + 1]
                + self.log_A,
                axis=2,
            )  # Insert additional axis to broadcast along the rows of the transition matrix
    ...
    def _check_obs(self, obs):
        ...
        assert (
            obs.shape[2] == self.n_dims
        ), f"Third dimension of obs must be equal to {self.n_dims}"

    def _check_gamma(self, gamma):
        gamma_sum = np.exp(gamma).sum(axis=1)
        assert np.allclose(
            gamma_sum, np.ones(gamma_sum.shape), atol=1e-6
        ), "Gamma probabilities must sum to 1 along the state axis"
