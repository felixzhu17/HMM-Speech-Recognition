...
class DNN(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.log_softmax(
            x, dim=-1
        )  # Apply log softmax to make output probabilities


class DNNHMM:
    def __init__(
        self, n_states, n_dims, n_encoding_dims=None, deterministic_start=True
    ):
    ...
    def m_step(self, obs, mask=None):
        ...
        for epoch in range(20):
            optimizer.zero_grad()
            emission_log_likelihood = self._get_emission_log_likelihood(
                obs, tensor=True
            )
            loss = criterion(
    ...
    def _check_mask(self, mask):
        for i in range(mask.shape[0]):
            for j in range(mask.shape[1]):
                assert np.all(mask[i, j, :]) or not np.any(
                    mask[i, j, :]
                ), "Third dimension is not all Trues or all Falses"
