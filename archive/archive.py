...
class BatchStableGMMHMM:
    ...
    def _init_mu(self, obs):
        ...
        subset = self.random_state.choice(
            np.arange(flattened_obs.shape[1]), size=self.n_states, replace=False
        )
    ...
    def m_step(self, obs):
        ...
        for s in range(self.n_states):
            self.mu[:, s] = (
                (gamma_s[:, s, :].reshape(n_batches, 1, n_observations) * obs)
                .sum(axis=2)
                .sum(axis=0)
            ) / gamma_s[:, s, :].sum()
        ...
        for s in range(self.n_states):
            ...
            self.covs[s, :, :] = (
                np.matmul(
                    gamma_s[:, s, :].reshape(n_batches, 1, n_observations) * diff,
        ...
        for i in range(self.n_states):
            for j in range(self.n_states):
                self.A[i, j] = np.exp(
                    np.logaddexp.reduce(self.xi[:, i, j, :].flatten())
    ...
    def _check_obs(self, obs):
        ...
        assert (
            obs.shape[1] == self.n_dims
        ), f"Second dimension of obs must be equal to {self.n_dims}"
