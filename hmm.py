...
class StableGMMHMM:
    ...
    def _init_mu(self, obs):
        ...
        subset = self.random_state.choice(np.arange(obs.shape[1]), size=self.n_states, replace=False)
        self.mu = obs[:, subset]
    ...
    def e_step(self, obs):
        ...
        for t in range(n_observations - 1): # Loop through every time step
            for i in range(self.n_states): # Loop through every start state
                ...
                self.xi[i, :, t] = self.alpha[i, t]  + self.log_A[i, :] + emission_log_likelihood[:, t + 1] + self.beta[:, t + 1]
    ...
    def m_step(self, obs):
        ...
        for i in range(self.n_states):
            for j in range(self.n_states):
                self.A[i, j] = np.exp(np.logaddexp.reduce(self.xi[i, j, :]) - np.logaddexp.reduce(self.gamma[i, :-1]))
    ...
    def forward(self, obs, emission_log_likelihood = None):
        ...
        for t in range(n_observations):
            if t == 0:
                ...
                alpha[:, t] = emission_log_likelihood[:, t] + np.logaddexp.reduce(self.log_A.T + alpha[:, t - 1], axis=1)
    ...
    def backward(self, obs, emission_log_likelihood = None):
        ...
        for t in range(n_observations - 1)[::-1]:
            ...
            beta[:, t] = np.logaddexp.reduce((beta[:, t+1] + emission_log_likelihood[:, t+1]) + self.log_A, axis=1)
        return beta
...
class RawGMMHMM:
    ...
    def e_step(self, obs):
        ...
        for t in range(n_observations - 1): # Loop through every time step
            for i in range(self.n_states): # Loop through every start state
                # Compute transition probability 
                self.xi[i, :, t] = self.alpha[i, t] * self.A[i, :] * emission_likelihood[:, t + 1] * self.beta[:, t + 1] / likelihood
    ...
    def m_step(self, obs): 
        ...
        for s in range(self.n_states):
            diff = obs - self.mu[:, s].reshape(-1, 1)
            self.covs[s, :, :] = np.dot(self.gamma[s] * diff, diff.T) / self.gamma[s].sum()
            self.covs[s, :, :] += np.eye(self.n_dims) * 1e-6  # Add a small value to avoid numerical underflow
